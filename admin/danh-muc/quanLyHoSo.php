  ...
  <section class="admin mx-auto items-center ">
    ...
    <section class="manage grid grid-cols-5 gap-4 my-4 ">
      <div class="menu  leading-10 bg-[#30536D]">
        ...
        <a class="block py-[35px] font-bold text-lg text-white to-yellow-300 hover:bg-white hover:text-green-300 hover:border hover:border-red-500" href="../../doctor/quanLyKetLuan.php"><i class="fa-solid fa-comment-medical p-4"></i>Kết luận của bác sĩ</a>
      </div>
      <section class="col-span-4 border-2-sm bg-gray-200 shadow-inherit">
        <h2 class="py-6 font-bold text-3xl text-center text-gray-600 uppercase">Quản lý Hồ Sơ Bác Sĩ</h2>
          ...
          <table class="w-full texts-sm text-left p-3 ">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400 ">
              <tr>
                ...
                <th class="py-3 px-6"> <a href="quanLyHoSo/add.php" class="border rounded-md px-4 py-2 bg-green-500 text-center text-white font-bold hover:text-green-400 hover:bg-white hover:border-red-300">Thêm tài khoản</a> </th>

              </tr>
            </thead>
            <tbody class="bg-white border-b dark:bg-gray-300 dark:border-gray-700">

              <?php foreach ($doctor as $index => $bs) : ?>
                <tr>
                  <td class="py-4 px-6"><?= $index + 1 ?></td>
                  <td class="py-4 px-6"><?= $bs['name'] ?></td>
                  <td class="py-4 px-6"><?= $bs['img'] ?></td>
                  <td class="py-4 px-6"><?= $bs['information'] ?></td>
                  <td class="py-4 px-6">
                    <a class="border rounded-md px-4 py-2 bg-green-500 text-white font-bold hover:text-green-400 hover:bg-white hover:border-red-300" href="quanLyHoSo/update.php?id=<?= $bs['id']?>">Sửa</a>
                    <a onclick="return confirm('Xác nhận xóa?')" class="border rounded-md px-4 py-2 bg-green-500 text-white font-bold hover:text-green-400 hover:bg-white hover:border-red-300" href="quanLyHoSo/delete.php?id=<?= $bs['id']?>">Xóa</a>
                  </td>
