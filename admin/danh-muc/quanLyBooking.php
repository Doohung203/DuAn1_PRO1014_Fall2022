...
//show
$sql = " SELECT booking.id, user.hoten, user.sdt,user.diachi, schedule.time, doctor.name as doctor, service.name as service FROM booking INNER JOIN user on booking.id_user = user.id INNER JOIN schedule on booking.id_schedule = schedule.id INNER JOIN doctor on booking.id_doctor = doctor.id INNER JOIN service on booking.id_service = service.id ";
  ...
  <section class="admin mx-auto items-center ">
    ...
    <section class="manage grid grid-cols-5 gap-4 my-4 ">
      <div class="menu  leading-10 bg-[#30536D]">
        ...
        <a class="block py-[35px] font-bold text-lg text-white to-yellow-300 hover:bg-white hover:text-green-300 hover:border hover:border-red-500" href="../..//danh-muc/quanLyKetLuan.php"><i class="fa-solid fa-comment-medical px-8"></i>Kết luận của bác sĩ</a>
      </div>
