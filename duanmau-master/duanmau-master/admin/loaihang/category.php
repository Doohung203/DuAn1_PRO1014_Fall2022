            ...
            <tbody>
                ...
                foreach (selectDb("SELECT * FROM category") as $item) { ?>
                    <tr>
                        ...
                        <td><?= $item['name'] ?></td>
                        <td>
                            <a href="../loaihang/editCate.php?id=<?= $item['id'] ?>" class="btn btn-danger">Sửa</a>
                            <a href="category.php?id=<?= $item['id'] ?>" class="btn btn-primary" onclick="return confirm('Bạn có chắc chắn muốn xoá?')">Xóa</a>
                        </td>
