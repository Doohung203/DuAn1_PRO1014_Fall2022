...
<div class="col-xs-9 col-sm-9 col-md-9 col-lg-9">
	...
	<a href="../hanghoa/addPro.php" class="btn btn-success">Thêm mới sản phẩm</a>
			...
			foreach (selectDb("SELECT * FROM product ORDER BY id DESC LIMIT $tin,$data") as $item) {
				foreach (selectDb("SELECT * FROM category WHERE id=" . $item['id_cate']) as $row) {
					...
					<tr>
						...
						<td>
							<a href="../hanghoa/editPro.php?id=<?=$item['id'] ?>" class="btn btn-danger">Sửa</a>
							<a href="product.php?id=<?= $item['id'] ?>" class="btn btn-primary" onclick="return confirm('Bạn chắc chắn muốn xóa chứ?')">Xóa</a>
						</td>
					</tr>
			<?php
				}
